...
func TestRegisterRoutes_UsesStructuredErrorEnvelope(t *testing.T) {
	...
	RegisterRoutes(group, router.NewManager(db), db, nil)
...
func TestRegisterRoutes_MeReportsCertificateFeatureAvailability(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			group := engine.Group("/_authgate/api")
			group.Use(auth.AuthMiddleware(db))
			RegisterRoutes(group, router.NewManager(db), db, tc.certSvc)
...
func TestRegisterRoutes_RejectsCertificateCreateWhenDNSCredentialsAreMissing(t *testing.T) {
	gin.SetMode(gin.TestMode)
	auth.ConfigureJWTSecret("test-secret")
	...
	group := engine.Group("/_authgate/api")
	group.Use(auth.AuthMiddleware(db))
	RegisterRoutes(group, router.NewManager(db), db, &certservice.Service{})
	...
	if resp.Body.String() != "{\"error\":{\"code\":\"invalid_provider\",\"message\":\"cloudflare: api_token is required\"}}" {
		t.Fatalf("body = %s", resp.Body.String())
	}
}

func TestRegisterRoutes_RejectsCertificateCreateWhenDNSCredentialsAreWhitespaceOnly(t *testing.T) {
	gin.SetMode(gin.TestMode)
	auth.ConfigureJWTSecret("test-secret")
...
func TestRegisterRoutes_RejectsCertificateCreateWhenNameIsWhitespaceOnly(t *testing.T) {
	gin.SetMode(gin.TestMode)
	auth.ConfigureJWTSecret("test-secret")
	...
	if resp.Body.String() != "{\"error\":{\"code\":\"invalid_name\",\"message\":\"certificate name required\"}}" {
		t.Fatalf("body = %s", resp.Body.String())
	}
}

func TestRegisterRoutes_RejectsCertificateCreateWhenDNSProviderIsUnsupported(t *testing.T) {
	gin.SetMode(gin.TestMode)
	auth.ConfigureJWTSecret("test-secret")
	...
	if resp.Body.String() != "{\"error\":{\"code\":\"invalid_provider\",\"message\":\"unsupported DNS provider: pdns\"}}" {
		t.Fatalf("body = %s", resp.Body.String())
	}
}
...
func TestRegisterRoutes_UpdateLegacyStoredAuthRuleTypeNormalizesResponseAndPreservesCredentials(t *testing.T) {
	gin.SetMode(gin.TestMode)
	auth.ConfigureJWTSecret("test-secret")
	...
	if updatedRule.Type != "basic" {
		t.Fatalf("updatedRule.Type = %q, want %q", updatedRule.Type, "basic")
	}
	if updatedRule.Config.Username != "service-user-updated" {
		t.Fatalf("updatedRule.Config.Username = %q, want %q", updatedRule.Config.Username, "service-user-updated")
	}
	if updatedRule.Config.Password != "shared-password" {
		t.Fatalf("updatedRule.Config.Password = %q, want preserved password", updatedRule.Config.Password)
	}
	...
	if payload["type"] != "basic" {
		t.Fatalf("response type = %v, want %q", payload["type"], "basic")
	}
	config, ok := payload["config"].(map[string]any)
	if !ok {
		t.Fatalf("response config = %v, want object", payload["config"])
	}
	if config["username"] != "service-user-updated" {
		t.Fatalf("response config.username = %v, want %q", config["username"], "service-user-updated")
	}
	if _, hasPassword := config["password"]; hasPassword {
		t.Fatalf("response leaked password in config: %v", config)
	}
}
...
func TestRegisterRoutes_CreateBasicAuthRuleRejectsWhitespaceOnlyPassword(t *testing.T) {
	gin.SetMode(gin.TestMode)
	auth.ConfigureJWTSecret("test-secret")
	...
	if !bytes.Contains(resp.Body.Bytes(), []byte(`"code":"missing_basic_credentials"`)) {
		t.Fatalf("body = %s, want missing_basic_credentials", resp.Body.String())
	}
}

func TestRegisterRoutes_UpdateBasicAuthRulePreservesWhitespaceOnlyPassword(t *testing.T) {
	gin.SetMode(gin.TestMode)
	auth.ConfigureJWTSecret("test-secret")
