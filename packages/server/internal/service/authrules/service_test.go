...
func TestServiceCreateAuthRule_AutoGeneratesAPIKeySecret(t *testing.T) {
	db := newTestDB(t)
	...
	if rule.Config.Secret == "" {
		t.Fatal("Create() secret = empty, want auto-generated secret")
	}
	if len(rule.Config.Secret) != 64 {
		t.Fatalf("Create() secret length = %d, want 64 (32 bytes hex)", len(rule.Config.Secret))
	}
}
...
func TestServiceUpdateAuthRule_PreservesSecretWhenOmitted(t *testing.T) {
	...
	autoGeneratedSecret := created.Config.Secret
	if autoGeneratedSecret == "" {
		t.Fatal("Create() secret = empty, want auto-generated")
	}

	updated, err := svc.Update(created.ID, UpdateInput{
		RouteID: stringPtr("route-1"),
		Type:    stringPtr("apikey"),
	})
	if err != nil {
		t.Fatalf("Update() error = %v", err)
	}

	if updated.Config.Secret != autoGeneratedSecret {
		t.Fatalf("updated.Config.Secret = %q, want preserved secret %q", updated.Config.Secret, autoGeneratedSecret)
	}
}
...
func TestServiceUpdateAuthRule_PreservesOmittedTypeRouteAndHeaderState(t *testing.T) {
	...
	createdSecret := created.Config.Secret
	...
	if updated.Config.Secret != createdSecret {
		t.Fatalf("updated.Config.Secret = %q, want preserved secret %q", updated.Config.Secret, createdSecret)
	}
}
