...
func TestService_ProvisionLocal(t *testing.T) {
	...
	cert, err := svc.ProvisionLocal(context.Background(), "wildcard", "*.example.com", 0, nil)
	if err != nil {
...
func TestService_ProvisionLocal_DuplicateDomain(t *testing.T) {
	...
	if _, err := svc.ProvisionLocal(context.Background(), "first", "foo.example.com", 0, nil); err != nil {
		t.Fatalf("first: %v", err)
	}
	_, err := svc.ProvisionLocal(context.Background(), "second", "foo.example.com", 0, nil)
...
func TestService_Resign_LocalCA(t *testing.T) {
	svc, _, db, rel := newTestSetup(t)
	cert, _ := svc.ProvisionLocal(context.Background(), "wildcard", "*.example.com", 0, nil)
